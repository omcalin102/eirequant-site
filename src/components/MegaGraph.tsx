...
> = {
    pid: {
        title: "EQX-M1 PID",
        body:
            "The PID path applies the model's exposure controller to reduce risk when conditions weaken. Read the window-specific statistics above alongside the chart: a shallower drawdown or lower volatility may come with meaningful opportunity cost when broad equities trend strongly.\n\nThe comparison is most useful for studying regime sensitivity and the timing of exposure changes. It does not establish that the historical trade-off will persist, and it should be considered alongside turnover, transaction costs and calibration uncertainty.",
    },
    baseline: {
        title: "EQX-M1 Baseline",
        body:
            "The Baseline path shows the model without the additional PID exposure control. Comparing it with both the PID path and the benchmark helps isolate how much of each window's return, volatility and drawdown behaviour is associated with the controller.\n\nTreat differences cautiously: results can depend on the selected period and market regime. Turnover, transaction costs, calibration choices and the timing of exposure reductions all affect whether an apparent improvement remains useful out of sample.",
    },
};
